                <div class="slimscroll-menu" id="remove-scroll">
                    <!--- Sidemenu -->
                    <div id="sidebar-menu">
                        <!-- Left Menu Start -->
                        <ul class="metismenu" id="side-menu">
                            <li class="menu-title">Informasi</li>
                            <li>
                                <a href="<?= base_url('Partner') ?>" class="waves-effect">
                                    <i class="mdi mdi-view-dashboard"></i> <span> Dashboard </span>
                                </a>
                            </li>
                
                            <li>
                                <a href="<?= base_url('') ?>" class="waves-effect">
                                    <i class="mdi ion-person"></i><span> Profile </span>
                                </a>
                            </li>
                
                            <li>
                                <a href="<?= base_url('') ?>" class="waves-effect">
                                    <i class="ti-bell noti-icon"></i><span class="badge badge-primary badge-pill float-right">2</span> <span> Notification </span>
                                </a>
                            </li>

                            <li class="menu-title">Aktivitas Cabang</li>

                            <li>
                                <a href="javascript:void(0);" class="waves-effect"><i class="mdi mdi-email-outline"></i><span> Leads <span class="float-right menu-arrow"><i class="mdi mdi-chevron-right"></i></span> </span></a>
                                <ul class="submenu">
                                    <li><a href="email-inbox.html">Database</a></li>
                                    <li><a href="email-read.html">Prospect</a></li>
                                </ul>
                            </li>

                            <li>
                                <a href="javascript:void(0);" class="waves-effect"><i class="mdi ion-person-stalker "></i><span> Kerjasama <span class="float-right menu-arrow"><i class="mdi mdi-chevron-right"></i></span> </span></a>
                                <ul class="submenu">
                                    <li><a href="email-inbox.html">Mapping Lokasi Partner</a></li>
                                    <li><a href="email-read.html">Partnership</a></li>
                                    <li><a href="email-compose.html">Agent / BA</a></li>
                                </ul>
                            </li>

                            <li class="menu-title">Bantuan HD</li>

                            <li>
                                <a href="<?= base_url('') ?>" class="waves-effect">
                                    <i class="ion-headphone  noti-icon"></i><span> Product Support </span>
                                </a>
                            </li>

                            <li>
                                <a href="<?= base_url('') ?>" class="waves-effect">
                                    <i class="ion-ios7-paper-outline noti-icon"></i><span> NST </span>
                                </a>
                            </li>

                
                        </ul>

                        
                
                    </div>
                    <!-- Sidebar -->
                    <div class="clearfix"></div>
                
                </div>